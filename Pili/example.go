...
var (
	AccessKey = "28YojThUOGZI1jhdYCHwVWs_MB1IkBeNgmrqACBB"            // Kodo 账号的 AccessKey.
	SecretKey = "itJeSnuh0ZbcKlZM6KTZHDRGwTCIBqAOaNNQoqQ_"            // Kodo 账号的 SecretKey.
	HubName   = "qingting-zhibo" // Hub 必须事先存在.
)
...
func historyActivity(hub *pili.Hub, key string) {
	stream := hub.Stream(key)
	records, err := stream.HistoryActivity(1611720000, 1611721360)
	if err != nil {
		return
	}
	fmt.Println(records)
